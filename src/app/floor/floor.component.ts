...
export class FloorComponent implements OnInit {  
  ...
  getList() {
    ...
    this._service.get('floor/list', obj).subscribe(res => {
      console.log(res);
      if (!res.Success) {
  ...
  getItem(id) {
    ...
    this._service.get('floor/get/' + id).subscribe(res => {
      this.blockUI.stop();
      if (!res.Success) {
        this.toastr.error(res.Message, 'Error!', { timeOut: 2000 });
        return;
      }
      this.formTitle = 'Update Floor';
  ...
  onFormSubmit() {
    ...
    const request = this._service.post('floor/create-or-update', obj);
