...
getBillEligibleRetailersCountData() {
  ...
  this._service.get('bill-dashboard/bill-eligible-retailers-count',obj).subscribe(res => {
    this.blockUI.stop();
    if (!res) {
      this.toastr.error(res.Message, 'Error!', { closeButton: true, disableTimeOut: true });
      return;
...
getRetailersPeripheralsCountData() {
  ...
  this._service.get('bill-dashboard/retailers-peripherals-count',obj).subscribe(res => {
    this.blockUI.stop();
    if (!res) {
      this.toastr.error(res.Message, 'Error!', { closeButton: true, disableTimeOut: true });
      return;
...
getRetailersBillRecordsData() {
  ...
  this._service.get('bill-dashboard/retailers-bill-records',obj).subscribe(res => {
    this.blockUI.stop();
    if (!res) {
      this.toastr.error(res.Message, 'Error!', { closeButton: true, disableTimeOut: true });
      return;
...
getBillEligibleWholesalersCountData() {
  ...
  this._service.get('bill-dashboard/bill-eligible-wholesalers-count',obj).subscribe(res => {
    this.blockUI.stop();
    if (!res) {
      this.toastr.error(res.Message, 'Error!', { closeButton: true, disableTimeOut: true });
      return;
...
getWholesalersPeripheralsCountData() {
  ...
  this._service.get('bill-dashboard/wholesalers-peripherals-count',obj).subscribe(res => {
    this.blockUI.stop();
    if (!res) {
      this.toastr.error(res.Message, 'Error!', { closeButton: true, disableTimeOut: true });
      return;
...
getWholesalersBillRecordsData() {
  ...
  this._service.get('bill-dashboard/wholesalers-bill-records',obj).subscribe(res => {
    this.blockUI.stop();
    if (!res) {
      this.toastr.error(res.Message, 'Error!', { closeButton: true, disableTimeOut: true });
      return;
...
showPicChart() {
  ...
  if(this.retailersBillRecords && this.wholesalersBillRecords){
    ...
    let collectableAmount = this.retailersBillRecords.retailer_collectable_bill_amount + this.wholesalersBillRecords.wholesaler_collectable_bill_amount;
    ...
    let due = collectableAmount - collectedAmount;
      ...
      series: [{
       ...
       //name: 'Amount Collection Pie Chart',
        ...
        dataLabels: {
          enabled: true,
          formatter: function () {
            return Math.round(Math.round(this.percentage * 100) / 100) + ' %';
          },
          distance: -20,
          color: 'black',
          style: {
