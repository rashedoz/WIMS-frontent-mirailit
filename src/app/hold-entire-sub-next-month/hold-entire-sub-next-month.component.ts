...
export class HoldEntireSubNextMonthComponent implements OnInit {
  ...
  modalTitle = "Add ";
  btnSaveText = "Hold Entire Subscription From Next Month";
  ...
  onFormSubmit() {
    ...
    this.confirmService.confirm('Are you sure?', 'You are holding the entire subscription from next month .')
    .subscribe(
        result => {
            if (result) {
              this._service.post('subscription/hold-entire-subscription-from-next-month', obj).subscribe(
                data => {
                  this.blockUI.stop();
