...
export class InvoiceLogComponent implements OnInit {
  ...
  onGenerateAction() {
    ...
    this.confirmService.confirm("Are you sure?", "You are generating the bill.").subscribe((result) => {
      if (result) {
        const request = this._service.post('bill/generate-monthly-invoice-all-customer',obj);
        request.subscribe(
          (data) => {
            this.blockUI.stop();
  ...
  getList() {
    ...
    const obj:any ={
      ...
      if(this.bsLogRangeValue[0]){
        obj.invoice_start_date = moment(this.bsLogRangeValue[0]).format('YYYY-MM-DD'),
        obj.invoice_end_date = moment(this.bsLogRangeValue[1]).format('YYYY-MM-DD')
      }



    this._service.get('bill/invoice-log-list',obj).subscribe(res => {
